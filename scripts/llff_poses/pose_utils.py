...
def gen_poses(basedir, match_type, factors=None):
    ...
    #@limengxun: copy all images* directory from upper level to current level
    basedir_upper = os.path.dirname(basedir)
    for d_name in ['images', 'images_2', 'images_4', 'images_8']:
        imgdir = os.path.join(basedir_upper, d_name)
        imgdir_new = os.path.join(basedir, d_name)
        if os.path.exists(imgdir) and not os.path.exists(imgdir_new):
            shutil.copytree(imgdir, imgdir_new)
