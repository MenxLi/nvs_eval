...
import tqdm
import tyro
from models.gsplat.examples.datasets.colmap import Dataset, Parser
from models.gsplat.examples.datasets.traj import generate_interpolated_path
from torch import Tensor
from torch.utils.tensorboard import SummaryWriter
from torchmetrics.image import PeakSignalNoiseRatio, StructuralSimilarityIndexMeasure
from torchmetrics.image.lpip import LearnedPerceptualImagePatchSimilarity
from models.gsplat.examples.utils import (
    AppearanceOptModule,
    CameraOptModule,
    colormap,
    knn,
...
exp_name = os.environ.get("EXP_NAME", "simple_trainer_2dgs")
exp_home = Path("DATA") / "output" / exp_name / "2dgs"
...
class Config:
    ...
    app_opt_lr: float = 4e-3
    # Regularization for appearance optimization as weight decay
    app_opt_reg: float = 4e-6
...
class Runner:
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        if not self.cfg.disable_viewer:
            raise NotImplementedError("Viewer is not supported in this script")
...
def main(cfg: Config):
    ...
    for i in range(10):
        render_image_file = result_dir / "renders" / f"val_{i:04d}.png"
        shutil.copy(render_image_file, dst_render_dir / f"eval_img_{i:04d}.png")

    val_json = result_dir / "stats" / f"val_step{cfg.max_steps - 1}.json"
    val_json = json.loads(val_json.read_text())
    val_metric_psnr = val_json["psnr"]
    val_metric_ssim = val_json["ssim"]
    val_metric_lpips = val_json["lpips"]
    val_metric_time = val_json["ellipse_time"]
    val_metric_fps = 1 / val_metric_time

    with open(exp_home / "eval.json", "w") as f:
        json.dump(
            {
                "expname": exp_name + "_2dgs",
                "results":{
                    "psnr": val_metric_psnr,
                    "ssim": val_metric_ssim,
                    "lpips": val_metric_lpips,
                    "fps": val_metric_fps,
                }
            },
            f,
